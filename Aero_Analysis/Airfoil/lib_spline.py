...
def draw_bspline(C=None, P=None, n=None, V_type=None, endpoint_epsilon=0.00001):
    ...
    if C:
        ...
        print('yes')
        curvepts = np.asarray(curvepts)
        # curvepts.reshape(len(curve(pts)/2), 2)
        print(curvepts)
...
def getPoints(C=None, P=None, n=None, V_type=None, endpoint_epsilon=0.00001):
    """Helper function to draw curves."""
    if P.any() and n and V_type:
        C = C_factory(P, n, V_type)
    if C:
...
def getThickness(X, curvepts):
    top = curvepts[0:len(curvepts)/2, :]
    bottom =  curvepts[len(curvepts)/2:, :]

    top= top[::-1]
    top_thick = np.asarray(np.interp(X,top[:,0], top[:,1]))
    bottom_thick = np.asarray(np.interp(X,bottom[:,0], bottom[:,1]))

    thickness = top_thick - bottom_thick

    return thickness

def getThickness_cs(X, curvepts):
    # print(curvepts)
    curvepts_real = np.real(curvepts)
    curvepts_imag = np.imag(curvepts)

    thick_real = getThickness(X,curvepts_real)
    thick_imag = getThickness(X,curvepts_imag)


    thickness =np.zeros(len(X), dtype=complex)
    for i in range(len(X)):
        thickness[i] = complex(thick_real[i], thick_imag[i])
    return thickness

def getArea(points):
    area = 0.0
    for i in range(len(points)-1):
        area += (points[i,0 ] - points[i +1,0 ])*(points[i,1] + points[i + 1,1])/2.0
