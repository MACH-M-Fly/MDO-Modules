...
class xfoilAnalysis():
    def __init__(self,  x = [], y = [], airfoil_file='none', re=1e5,mach=0.0,iter=50):
          # end if
        ...
        self.setCoordinates(np.real(self.x),np.real(self.y))
    ...
    def solveAlphaComplex(self,angle):
        ...
        # xfoil_cs.ci04.npan = self.Npane  # Iterations Limit Set
    ...
    def rePanel(self, N):
        ...
        # for i in dir(xfoil_cs):
        #     for j in getattr(xfoil_cs,i):
        #       print j,"  ",type(getattr(getattr(xfoil_cs,i),j))


        for i in dir(xfoil_cs):
            print i,"  ",type(getattr(xfoil_cs,i))

        print('\n')

        for i in dir(xfoil_cs.ci05):
            print i,"  ",type(getattr(xfoil_cs.ci05,i))
        # xfoil_cs.ci04.npan = N  # Iterations Limit Set
