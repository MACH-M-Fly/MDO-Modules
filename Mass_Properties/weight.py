...
class weight(Component):
	'''calculate aircraft weight,areas and CG'''
	...
	def __init__(self ): 
		...
		self.add_param('C_r',val=0.4,desc='Cord [m]')
		self.add_param('t2',val=0.4,desc='Cord [m]')
		self.add_param('t3',val=0.4,desc='Cord [m]')
		self.add_param('t4',val=0.4,desc='Cord [m]')
		self.add_param('t5',val=0.4,desc='Cord [m]')

		self.add_param('b_htail', val=1.0, desc='tail span')
		self.add_param('C_r_t',val=0.4,desc='Cord tail [m]')

		self.add_param('boom_len', val=0.6,desc='tail boom length')	
		self.add_param('dist_LG', val=0.15,desc='offset of lading gear location')

			
		# set up outputs
		self.add_output('mass', val= 3.0, desc='total weight [kg]')
		self.add_output('x_cg', val = 0.0,desc='x cg')
		self.add_output('z_cg', val = 0.0,desc='z cg')					
		self.add_output('Iyy', val = 0.0,desc='Total Vertical tail Reference Area [m^2]')
		self.add_output('Sref_wing',val=2.0,desc='wing area [m^2]')
		self.add_output('Sref_tail', val=2.0, desc='tail area [m^2]')
		self.add_output('MAC',val=0.4,desc='Mean Aerodynamic Chord [m]')



		self.add_output('Xle', val=[]*5, desc='X cor leading edge')
		self.add_output('Yle', val=[]*5,desc='Y cor leading edge')	  
		self.add_output('C', val=[]*5, desc ='Chord')	  
		self.add_output('Xle_t', val=[]*2, desc ='Tail X cord LE')
		self.add_output('Yle_t', val=[]*2, desc ='Tail Y cord LE')
		self.add_output('C_t', val=[]*2, desc ='Chord Tail')	 
		self.add_output('test_class', Surface(arg=7), desc ='Chord Tail')	 

		# self.add_output('Xle', val=[]*5, desc='X cor leading edge')
		# self.add_output('Yle', val=np.zeros(5),desc='Y cor leading edge')	  
		# self.add_output('C', val=np.zeros(5), desc ='Chord')	  
		# self.add_output('Xle_t', val=np.zeros(2), desc ='Tail X cord LE')
		# self.add_output('Yle_t', val=np.zeros(2), desc ='Tail Y cord LE')
		# self.add_output('C_t', val=np.zeros(2), desc ='Chord Tail')	  
	...
	def solve_nonlinear(self,params,unknowns,resids):
		...
		# plt.plot(  wing_pos, wing_edge ,  'b-')#,  Yle, [sum(x) for x in zip(Xle, C)]],  'r')
		# plt.plot(  tail_pos, tail_edge ,  'r-')#,  Yle, [sum(x) for x in zip(Xle, C)]],  'r')
		# plt.plot([0, 0], [C[0], Xle_t[0]], 'g-')
